...
function MyTasksSidebar({ searchKeyword, refreshKey }) {
    const currentUser = useSelector(state => state.user);
    const [showMyTasks, setShowMyTasks] = useState(false);
    const [myTasks, setMyTasks] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                let visibleTasks = tasks.filter(t => activeProjectIds.includes(t.projectId));
                ...
                if (currentUser.role === "LEADER") {
                    ...
                    visibleTasks = visibleTasks.filter(t => t.assignToUserId === currentUser.userId);
                }
        ...
        fetchData();
    }, [currentUser.userId, currentUser.role, currentUser.email, refreshKey]);

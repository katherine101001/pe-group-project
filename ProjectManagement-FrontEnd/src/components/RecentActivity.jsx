    ...
    const fetchTasks = async () => {
      try {
        ...
        const filteredTasks = allTasks.filter(task => {
          ...
          if (role === "LEADER") return task.projectLeaderId === userId;
          if (role === "MEMBER") return task.assignToUserId === userId;
          return false;
        });
          ...
          <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
            {tasks.map(task => {
              ...
              return (
                <Link
                  key={task.id}
                  to={`/app/taskDetails?projectId=${task.projectId}&taskId=${task.id}`} // <-- add projectId here
                  className="block p-6 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors rounded-lg"
                >

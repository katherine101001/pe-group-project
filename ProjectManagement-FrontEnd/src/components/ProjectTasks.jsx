  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        ...
        {(filters.status || filters.type || filters.priority || filters.assignee) && (
          <button onClick={() => setFilters({ status: "", type: "", priority: "", assignee: "" })} className="px-3 py-1 flex items-center gap-2 rounded bg-gradient-to-br from-purple-400 to-purple-500 text-zinc-100 dark:text-zinc-200 text-sm">
            <XIcon className="size-3" /> Reset
          </button>
        )}
      </div>
          ...
          <tbody>
            {filteredTasks.length > 0 ? (
              filteredTasks.map((task, idx) => {
                ...
                const { background, prioritycolor } = priorityTexts[task.priority] || {};
                return (
                  <tr key={task.id} className="border-t border-zinc-300 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-all cursor-pointer" onClick={() => navigate(`/app/taskDetails?projectId=${task.projectId}&taskId=${task.id}`)}>
